...
CREATE INDEX idx_users_average_rating ON users(average_rating DESC) WHERE user_type = 'prestador';
...
-- Location-based indexes
CREATE INDEX idx_services_city_state ON services(city, state);
CREATE INDEX idx_services_location ON services(latitude, longitude) WHERE latitude IS NOT NULL AND longitude IS NOT NULL;
...
CREATE INDEX idx_messages_conversation ON messages(service_id, created_at DESC);
...
CREATE INDEX idx_notifications_urgent ON notifications(user_id, is_urgent, created_at DESC) 
